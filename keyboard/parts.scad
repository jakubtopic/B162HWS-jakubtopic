...
/**
 * [keyboardBody description]
 * @param  {[type]} size       [description]
 * @param  {[type]} basemargin [description]
 * @return {[type]}            [description]
 */
module keyboardBody(size,basemargin) {
    hull()
        mirror([0,1,0])
            for(i=[0:1], j=[0:1])
        ...
        for(col = [0:len(layout[0])-1])
            if(len(layout[row][col]) > 0)
                translate(multiplyvec(size,[col,-row,0])) {
                    ksize = addvec(multiplyvec(keysize,[layout[row][col][0],layout[row][col][1],1]),
                            [(layout[row][col][0]-1)*offset, (layout[row][col][1]-1)*offset, 0]);
                    if(keycap)
...
module keyCapText(size, text, textsize, textz) {
    translate(multiplyvec(size,[.5,-.5,0]))
        linear_extrude(height=size[2]+textz)
            text(text, halign="center", valign="center", convexity=10, size=textsize);
}